...
//	Addons = ArrayList<String[]>();
	...
	public boolean isAddHerokuPostgres1() throws IOException {
		System.out.println("[HerokuDeployer] isAddHerokuPostgres1 guard .... HerokuPostgres availability = " + BIPDeployerOCCI.herokuPostgresAvailability.isAvailability());
	...
	public boolean isAddHerokuPostgres2() throws IOException {
		System.out.println("[HerokuDeployer] isAddHerokuPostgres2 guard .... HerokuPostgres availability = " + BIPDeployerOCCI.herokuPostgresAvailability.isAvailability());
		if (isInAddons("heroku-postgresql", Addons) && findPlan("heroku-postgresql", Addons).equals("hobby-basic") && BIPDeployerOCCI.herokuPostgresAvailability.isAvailability()) {
			return true;
		} else {
			return false;
		}
	...
	public boolean isAddClearDBMySQL1() throws IOException {
		System.out.println("[HerokuDeployer] isAddClearDBMySQL1 guard .... ClearDBMySQL availability = " + BIPDeployerOCCI.ClearDBMySQLAvailability.isAvailability());
		if (isInAddons("cleardb", Addons) && findPlan("cleardb", Addons).equals("ignite") && BIPDeployerOCCI.ClearDBMySQLAvailability.isAvailability()) {
			return true;
		} else {
			return false;
		}
	...
	public boolean isAddScoutAPM1() throws IOException {
		System.out.println("[HerokuDeployer] isAddScoutAPM1 guard .... ScoutAPM availability = " + BIPDeployerOCCI.ScoutAPMAvailability.isAvailability());
		if (isInAddons("scout", Addons) && findPlan("scout", Addons).equals("chair") && BIPDeployerOCCI.ScoutAPMAvailability.isAvailability()) {
			return true;
		} else {
			return false;
		}
	...
	public boolean isAddNewRelicAPM1() throws IOException {
		System.out.println("[HerokuDeployer] isAddNewRelicAPM1 guard .... NewRelicAPM availability = " + BIPDeployerOCCI.NewRelicAPMAvailability.isAvailability());
		if (isInAddons("newrelic", Addons) && findPlan("newrelic", Addons).equals("wayne") && BIPDeployerOCCI.NewRelicAPMAvailability.isAvailability()) {
			return true;
		} else {
			return false;
		}
	...
	public boolean isAddonAddingError() {
		...
		for (String addon[] : arr) {
			if (addon[0].equals("heroku-postgresql") && !BIPDeployerOCCI.herokuPostgresAvailability.isAvailability()) {
				System.out.println("[HerokuDeployer] addonAddingError guard .... HerokuPostgres can not be added!!!");
				re = true;
				break;
			} else if ( addon[0].equals("cleardb") && !BIPDeployerOCCI.ClearDBMySQLAvailability.isAvailability()) {
				System.out.println("[HerokuDeployer] addonAddingError guard .... ClearDBMySQL can not be added!!!");
				re = true;
				break;
			} else if ( addon[0].equals("jawsdb") && !BIPDeployerOCCI.NewRelicAPMAvailability.isAvailability()) {
				System.out.println("[HerokuDeployer] addonAddingError guard .... NewRelicAPM can not be added!!!");
				re = true;
				break;
			} else if ( addon[0].equals("scout") && !BIPDeployerOCCI.ScoutAPMAvailability.isAvailability()) {
				System.out.println("[HerokuDeployer] addonAddingError guard .... ScoutAPM can not be added!!!");
				re = true;
				break;
			} else if ( addon[0].equals("newrelic") && !BIPDeployerOCCI.NewRelicAPMAvailability.isAvailability()) {
	...
	@Transitions({
		...
		public void pushAppToContainer() {	
			...
			if (!conf.pushAppSkip) {
				...
				System.out.println("[HerokuDeployer-push] HerokuCli.warPath: " + HerokuCli.warPath);
				System.out.println("[HerokuDeployer-push] pushResponse: " + pushResponse);
				
				String url = StringUtils.substringBetween(pushResponse, "remote:", "deployed to Heroku");
				System.out.println("[HerokuDeployer-push] url: " + url);
				if (url == null) {
					pushAppError = true;
				} else {
	...
	@Transitions({
		@Transition(name = "toAppReady", source = "Init", target = "AppReady"),
	})
	public void toAppReady() {	
		System.out.println("[HerokuDeployer] spontaneous toAppReady triggered .... (Init --> TempAppReady)");
		System.out.println();
//		removeAddon("heroku-postgresql", Addons);
//		
//		conf.getDeployStatus().add("Add Heroku Postgres (hobby-dev)");
	...
	@Transitions({
		@Transition(name = "clickAddMorePostgres", source = "AppReady", target = "TempAppReady"),
	})
	public void clickAddMorePostgres() {	
		System.out.println("[HerokuDeployer] spontaneous clickAddMorePostgres triggered .... (AppReady --> TempAppReady)");
		System.out.println();
//		removeAddon("heroku-postgresql", Addons);
//		
//		conf.getDeployStatus().add("Add Heroku Postgres (hobby-dev)");
	...
	@Transitions({
		@Transition(name = "addMorePostgres", source = "TempAppReady", target = "AppReady", guard = "isAddMorePostgres"),
	})
	public void addMorePostgres() {	
		System.out.println("[HerokuDeployer] isAddMorePostgres triggered .... (AppReady --> AppReady)");
		System.out.println();
		removeAddon("heroku-postgresql", Addons);
	...
	@Guard(name = "isAddMorePostgres")
	public boolean isAddMorePostgres() throws IOException {
...
//		System.out.println("[HerokuDeployer] After: check conditions to add Postgres: " + isInAddons("heroku-postgresql", Addons) + " - " + findPlan("heroku-postgresql", Addons).equals("hobby-dev")  + " - " +  BIPDeployerOCCI.herokuPostgresAvailability.isAvailability());
		...
		if (isInAddons("heroku-postgresql", Addons) && findPlan("heroku-postgresql", Addons).equals("hobby-dev") && BIPDeployerOCCI.herokuPostgresAvailability.isAvailability()) {
//			System.out.println("[HerokuDeployer] GP: check conditions to add Postgres" + isInAddons("heroku-postgresql", Addons) + " - " + findPlan("heroku-postgresql", Addons).equals("hobby-dev")  + " - " +  BIPDeployerOCCI.herokuPostgresAvailability.isAvailability());
			return true;
		} else {
			return false;
		}
